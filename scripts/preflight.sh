...
export POSTGRES_DB="${POSTGRES_DB:-jobmatchkit}"
export DATABASE_URL="${DATABASE_URL:-postgresql://postgres:postgres@db:5432/jobmatchkit}"
...
export AUTH_SECRET_KEY="${AUTH_SECRET_KEY:-jobmatchkit-dev-secret-change-me}"
...
ALEMBIC_TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/jobmatchkit_preflight_alembic.XXXXXX")"
trap 'rm -rf "$ALEMBIC_TMP_DIR"; on_exit' EXIT
docker run --rm \
  -e UV_PROJECT_ENVIRONMENT=/tmp/uv-venv \
  -e DATABASE_URL=sqlite:////preflight/jobmatchkit_alembic.db \
  -v "$ROOT_DIR/backend:/app" \
